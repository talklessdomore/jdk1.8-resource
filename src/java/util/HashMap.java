...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 当桶上的结点数大于这个值时，链表会转成红黑树
     */
    static final int TREEIFY_THRESHOLD = 8;

    /**
     * 当桶上的结点数小于这个值时，红黑树树转链表
     */
    ...
    static final int hash(Object key) {
        /*
         * 计算hash值，让高16位和低16位 做异或操作，目的是尽量让32位的hash值在计算槽位的时候起作用，从而减少冲突
         */
        int h;
    ...
    /*******************************************************************
     ...
     补充：
     正数无符号右移：正数无符号右移是高位补0，移多少位补多少个0。比如：
    ...
    final void putMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
        ...
        if (s > 0) {
            if (table == null) {
                // s / loadFactor等于总容量个数，但是未什么要加1个呢？
    ...
    /**
     * 把value放入map中，并且建立映射关系
     *
     * @param hash key的hash值
     * @param key map的key
     * @param value 需要put到map中的值
     * @param onlyIfAbsent true不会改变已经存在的值  false则改变已经存在的值
     * @param evict true 非创建模式  false表示map处于创建模式
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            // 从这里看出，resize()也可以初始化map
            n = (tab = resize()).length;

        if ((p = tab[i = (n - 1) & hash]) == null)
            // 获取该元素应该放到哪一个桶上面，如果这个桶是空的，那么直接放进去就好了
            // (n - 1) & hash 使用位运算计算元素位于哪个桶，效率高。这种算法就要求桶的容量必须是2的次幂
     ...
     * 初始化或加倍表大小。如果为空，则分配
     * 与初始容量目标保持一致。
     * 否则，因为我们使用的是二次展开的幂，所以每个元素必须保持在同一索引中，或者移动
     * 在新表中使用两个偏移量的幂。
     * @return the table
     */
    final Node<K,V>[] resize() {
        ...
        /**
         * 假如通过 new HashMap(1000)，这时候新数组大小为1024
         */
     ...
     * @param hash key的hash值
     * @param key the key
     * @param value matchValue为true，则表示要用value进行匹配
     * @param matchValue true的话，表明value要相等才能移除
     * @param movable 如果为false，则在移除时不要移动其他节点。true则移动其他节点
     * @return the node, or null if none
     */
