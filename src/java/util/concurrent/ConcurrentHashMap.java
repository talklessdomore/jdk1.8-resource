...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 实际上保存的是ConcurrentHashMap中的元素个数
     * 利用CAS锁进行更新
     * 但它并不用返回当前ConcurrentHashMap的元素个数
     */
     ...
     * -1     代表正在初始化
     * -N     代表有N-1个线程正在进行扩容操作
     * 正数或0 代表hash表还没有被初始化，这个数值表示初始化或下一次进行扩容的大小
     */
    private transient volatile int sizeCtl;

    /**
     * 数组上桶的迁移工作已经进行到哪个位置的标识
     */
    ...
    public V get(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if ((eh = e.hash) == h) {
                 ...
                 *          调用TreeBin类的find方法，具体是以链表方式遍历还是红黑树方式遍历视情况而定
                 *  (2) ConcurrentHashMap正在扩容当中，并且该hash桶已迁移完毕，该位置被放置了ForwardingNode节点。
                 *          当前正在扩容，并且e这个节点的桶已经迁移完毕了，把请求转发到新数组上面
                 */
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            // 通过hash得到数组下标，并且获得桶的链表头
                ...
                synchronized (f) {
                    ...
                    // 查看下i桶上面的头结点还是不是f，是的话，说明没有其他线程进行修改了
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K,V> e = f;; ++binCount) {
                                ...
                                if (e.hash == hash &&
                                    ...
                                    // 如果key已经存在，根据onlyIfAbsent判断是否要更新值
                                    oldVal = e.val;
                                    if (!onlyIfAbsent)
                                        e.val = value;
                                    break;
    ...
    /**
     * ForwardingNode作用在扩容期间，当table[i]是个ForwardingNode节点时，代表该位置桶的所有节点已经移至新数组。
     * 因此get请求可以直接转发到新数组的i位置
     */
    static final class ForwardingNode<K,V> extends Node<K,V> {

        /**
         * 指向 扩容时新生成的数组
         * 如果扩容期间遇到新请求，就会转发到扩容后的新数组上
         */
    ...
    /**
     * 读取voaltile的sizeCtl 值赋给局部变量 sc = sizeCtl，之后当一个线程cas设置sizeCtl值为-1成功，之后的线程都将被拒绝，通过执行Thread.yield()。
     * 也就是说只允许一个线程执行初始化table 数组操作。sc == 0则table大小为16，否则就为sizeCtl大小的值。
     * 数组创建完成后sizeCtl = n - (n >>> 2)，相当于原先值的0.75，这之后sizeCtl代表阀值。
     */
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            if ((sc = sizeCtl) < 0)
                // 后面的线程发现是-1了，就让出CPU时间片
                Thread.yield();

            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                // 第一个线程把sizeCtl赋值为-1，这里利用CAS保证线程安全性
                try {
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        // sizeCtl = n - (n >>> 2)，相当于原先值的0.75，这之后sizeCtl代表阀值
                        sc = n - (n >>> 2);
                    }
                } finally {
                    sizeCtl = sc;
    ...
    /**
     * 新增元素时，也就是在调用 putVal 方法后
     * 为了通用，增加了个 check 入参，用于指定是否可能会出现扩容的情况
    ...
    private final void addCount(long x, int check) {
        ...
        // check一般是桶的个数，个数大于0了，就检查一下
        if (check >= 0) {

            Node<K,V>[] tab, nt; int n, sc;

            // 检查当前集合元素个数 s 是否达到扩容阈值 sizeCtl ，扩容时 sizeCtl 为负数，依旧成立，同时还得满足数组非空 且 数组长度不能大于允许的数组最大长度 这两个条件才能继续
            // 这个 while 循环除了判断是否达到阈值从而进行扩容操作之外还有一个作用就是当一条线程完成自己的迁移任务后，如果集合还在扩容，则会继续循环，继续加入扩容大军，申请后面的迁移任务
            while (s >= (long)(sc = sizeCtl) && (tab = table) != null &&
                   (n = tab.length) < MAXIMUM_CAPACITY) {

                // 该数组的扩容标识，若数组正在扩容，则此时sizeCtl的高16位将等于 rs
                int rs = resizeStamp(n);

                // sc < 0 说明集合正在扩容当中
                if (sc < 0) {

                    //判断扩容是否结束 或者 并发扩容线程数是否已达最大值，如果是的话直接结束while循环
                    ...
                    // rs << RESIZE_STAMP_SHIFT) + 2 表示此时只有一个线程在执行扩容
                    // sizeCtl之前代表阀值，更改后高16位为标识，低16位为扩容线程数加一


                    //  第二个参数为null会初始化新数组nextTable，这里确保nextTable数组初始化的安全性
                    transfer(tab, null);


                // 代表当前元素个数，并不是准确个数，之后分析ConcurrentHashMap的元素个数统计问题
                s = sumCount();
            }
        }
    }
    ...
    /**
     * 扩容状态下其他线程对集合进行插入、修改、删除、合并、compute等操作时遇到 ForwardingNode 节点会调用该方法，让当前线程帮助扩容
     */
    ...
    /**
     * 进行扩容操作。将每个桶中的节点移动或复制到新表中
     */
    private final void transfer(Node<K,V>[] tab, Node<K,V>[] nextTab) {

        // stride表示当前线程需要迁移的桶的个数
        ...
        /*
         * 新建一个占位对象，该占位对象的 hash 值为 -1。
         * 该占位对象存在时表示集合正在扩容状态，nextTable 属性指向扩容后的数组
         *
         * 该占位对象主要有两个用途：
         *   1、占位作用，用于标识数组该位置的桶已经迁移完毕，ConcurrentHashMap处于扩容中的状态。
